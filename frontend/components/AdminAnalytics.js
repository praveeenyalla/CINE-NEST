...
const AdminAnalytics = () => {
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(item => {
            ...
            if (filters.genre && filters.genre !== 'All Genres') {
                ...
                if (!itemGenres.includes(filters.genre)) return false;
            }
    ...
    // --- Enhance Data with Mock User Stats ---
    const enhancedData = useMemo(() => {
        return filteredData.map(item => {
            // Creative Mock: Views based on IMDb + Random factor (simulating popularity)
            const baseViews = (parseFloat(item.IMDb) || 5) * 10000;
            const randomFactor = Math.random() * 5000;
            const userViews = Math.floor(baseViews + randomFactor);
            return {
                ...item,
                userViews: userViews
            };
    ...
    const kpis = useMemo(() => {
        ...
        const totalMovies = enhancedData.filter(d => (d.Type || '').toLowerCase() === 'movie').length;
        const totalSeries = enhancedData.filter(d => ['series', 'tv show'].includes((d.Type || '').toLowerCase())).length;

        const imdbValues = enhancedData.map(d => parseFloat(d.IMDb) || 0).filter(v => v > 0);
        ...
        // User Data KPI
        const totalUserViews = enhancedData.reduce((acc, curr) => acc + (curr.userViews || 0), 0);
        const formattedTotalViews = totalUserViews > 1000000
            ? (totalUserViews / 1000000).toFixed(1) + 'M'
            : (totalUserViews / 1000).toFixed(1) + 'k';

        return { total, totalMovies, totalSeries, avgImdb, maxImdb, minImdb, formattedTotalViews };
    }, [enhancedData]);
    ...
    const platformChartData = useMemo(() => {
        ...
        return platforms.map(p => ({
            ...
            fullName: p,
            count: enhancedData.reduce((acc, item) => acc + (item[p] === 1 ? 1 : 0), 0),
            fill: BRAND_COLORS[p]
        }));
    }, [enhancedData]);
    ...
    // Top Genres by User Views (New Chart Data)
    const genreViewsChartData = useMemo(() => {
        const genreCounts = {};
        enhancedData.forEach(d => {
            if (d.Genres) {
                const genres = d.Genres.split(',').map(g => g.trim());
                // Split views across genres for accuracy or apply full views to each
                // Let's apply full views to simulate popularity in that genre
                const views = d.userViews || 0;
                genres.forEach(g => {
                    genreCounts[g] = (genreCounts[g] || 0) + views;
                });
            }
        });
    ...
    return (
        <div className="w-full text-white font-sans bg-transparent" id="admin-analysis-panel">
            <style jsx>{`
                .glass-panel {
            ...
            <div className="mb-8 flex items-end justify-between border-b border-white/5 pb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-2 ml-4">Advanced analytics and performance metrics for <code className="bg-white/5 px-1 py-0.5 rounded text-gray-300">Administrative Overview</code></p>
                </div>
            ...
            {/* Filters */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8 p-6 glass-panel rounded-xl">
                {/* Year Filter */}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                    ...
                    { title: "Total Views", value: kpis.formattedTotalViews, icon: "visibility", color: "text-cyan-400" }, // New KPI
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                {/* User Views by Genre (New Chart) */}
                <div className="glass-panel p-6 rounded-xl">
                    <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                        <span className="material-symbols-outlined text-cyan-400">group</span>
                        Top Genres by User Views
                    </h3>
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={genreViewsChartData}>
                                <defs>
                                    <linearGradient id="colorGenreViews" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="0%" stopColor="#0ea5e9" stopOpacity={0.8} />
                                        <stop offset="100%" stopColor="#0ea5e9" stopOpacity={0.2} />
                                    </linearGradient>
                                </defs>
                                <CartesianGrid strokeDasharray="3 3" stroke="#333" vertical={false} />
                                <XAxis dataKey="name" stroke="#666" tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} dy={10} />
                                <YAxis stroke="#666" tick={{ fontSize: 10, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
                                <Tooltip
                                    cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                                    contentStyle={{ backgroundColor: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', color: '#fff' }}
                                    itemStyle={{ color: '#fff' }}
                                />
                                <Bar dataKey="value" fill="url(#colorGenreViews)" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
